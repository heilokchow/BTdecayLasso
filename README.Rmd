...
Bradley-Terry model is used for ranking in sports tournament. Given the standard Bradley-Terry model,
we use an exponential decay rate to weight its log-likelihood function and apply Lasso penalty to achieve
a variance reduction and team grouping.
...
First, given raw datasets (five columns are home teams, away teams, home wins, away wins, time until now),
we convert this dataset into a dataframe which can be used for other function's input.

```{r eval = FALSE}
NFL <- BTdataframe(NFL2010)
```

Then, we comput the whole Lasso path for further analysis's use. In this example, to track the dynamically
changing abilities, we set 'decay.rate' to be 0.005. A higher decay rate will give more unbiased results for
current abilites' estimation with a side effect of higher variance.
